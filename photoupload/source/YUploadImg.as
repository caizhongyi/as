...
/*   presented by yukon12345  (http://hi.baidu.com/yukon_kanzaki/blog/item/b500fd1f77d808f8e1fe0b88.html) 
	...
	*/
	public class YUploadImg extends Sprite {
		private var sizeLimit=2*1024*1024 //2M上传限制（此参数可更改设置）
		private var _upURL:String;  
		...
		private var _notice                 //提醒消息文本容器
		private var _statusTex:TextField;     //状态显示文本               
		private var _urlRequest;
		private var _UIWidth = 666 ;             //整个上传工具的宽度高度
		private var _UIHeight=330;            
		private var _headH=0;                //头部。从最顶到灰色标题栏
		private var _imgShower;               //装载缩略图组的容器。即灰色线框
		private var _imgShowerH=328;          //容器高度
		...
		private var _photoCapacity		//相册容量
		...
		public function YUploadImg() {
			...
			if(param.width)
			_UIWidth = param.width;
			if(param.height)
			_UIHeight = param.height;
			_upURL = param["url"] ||  'http://ku.766.com/upload/index/game_id/2324/type/1.html';
			_photoCapacity = param["photo_capacity"] || 1024 * 1024;
		...
		private function  makeHead(){//头部分容器
			...
			*/
			...
			_statusTex.x=10;
			_statusTex.y=_headH+35
			_statusTex.height=20;
			_statusTex.width=500;
			_statusTex.text="0 张照片等待上传(共"+_numLimit+"张)"
			////addChild(_statusTex)
		...
		private function delImg(_tmp_index){//按索引删除图片
			...
			while(_tmp_i<_tmp_num){//重新排列剩下的图片,item索引index也要记得改变
				...
				_imgShower.getChildAt(_tmp_i).x=10+(_tmp_i%5)*(_UIWidth/5)
				_imgShower.getChildAt(_tmp_i).y=68+Math.floor(_tmp_i/5)*128
				_tmp_i++
			}
		 ...
		 * 统计容量
		 */
		private function countCapacity(){
			var cc_len = _fileQueue==null?0:_fileQueue.length;
			var cc_total_size = 0;
			if(cc_len){
				for(var cc_n:int = 0;cc_n<cc_len;cc_n++){
					cc_total_size = cc_total_size + _fileQueue[cc_n].size;
				}
			}
			
			cc_total_size = cc_total_size;
			////var _capacity = ExternalInterface.call("photoCapacity");
			if(_photoCapacity - cc_total_size>0 && cc_len){
				if(!contains(_uploadBu)) addChild(_uploadBu);
			}else{
				if(contains(_uploadBu)) removeChild(_uploadBu)
			}
			if(_photoCapacity - cc_total_size>0){
			   _statusTex.text=cc_len+" 张照片等待上传(共"+_numLimit+"张)"
			}else{
				_statusTex.text="相册容量不够！"
			}
		}
		...
		private function onFRL(e:Event){//当文件浏览框的按钮
			...
			{   
				...
				{    removeChild(_selectBu)
					...
					if(Math.ceil(_tmp_num/5)>2){//超过2列
					...
					while(_tmp_i<_tmp_num){  //遍历可添加的文件引用，放入_imgShower图片组显示容器
						...
						_tmp_file.x=10+(_tmp_i%5)*(_UIWidth/5) //坐标计算
						_tmp_file.y=68+Math.floor(_tmp_i/5)*128 
					 ...
					 if(_fileQueue.length==_numLimit)//如果原本就满了
					 {
						...
						_tmp_file.x=10+((_tmp_i+_tmp_hasNum)%5)*(_UIWidth/5) 
						_tmp_file.y=68+Math.floor((_tmp_i+_tmp_hasNum)/5)*128
		  ...
		  _tipsObj.tipsText.selectable=false //动态文字不可被选择
		   ...
		   addEventListener(Event.ENTER_FRAME,moveTips) //缓冲移动
			...
			private function moveTips(e:Event){//提示条跟随鼠标
				  ...
				  {  var targetX=this.mouseX+70
				     var targetY=this.mouseY+70
					  if(stage.mouseX>512)
					    targetX=this.mouseX-70
					   if(stage.mouseY>386)
					   targetY=this.mouseY-70
					   
				   _tipsObj.x+=(targetX-_tipsObj.x)*0.1  //缓动公式
				    _tipsObj.y+=(targetY-_tipsObj.y)*0.1
		...
		private function onUploadBu(e:Event){//点击上传
			...
			*/
				var checkresult = ExternalInterface.call("submitCheck") || true;
			...
			if(!contains(_sSelectBu))
			    addChild(_sSelectBu)
			if(!contains(_uploadBu))
			    addChild(_uploadBu)
			...
			private function setEmptyState(){//空图片状态时
				...
				if(contains(_cleanUpBu)) removeChild(_cleanUpBu)
				if(!contains(_selectBu))addChild(_selectBu)
				if(contains(_uploadBu)) removeChild(_uploadBu)
			}
			...
			private function onWaitItemSend(e:Event){//分步一张一张上传图片。等待上一张的上传完毕
				_statusTex.text="正在上传第"+_uploadingItem.index+"张图片(共"+_fileQueue.length+"张)"
		...
		private function continueUpload(_tmp_index){//上传下一张
			if(_tmp_index<_fileQueue.length){
				...
				ExternalInterface.call("beginUpload",_uploadingItem._urlRequest.url);
				_uploadingItem.addEventListener("itemSendOver",onWaitItemSend)
				_uploadingItem.upload()
			}
			else{
				_statusTex.text="共"+_fileQueue.length+"张图片上传完毕"
				ExternalInterface.call("submitComplete");
			}
		}
		...
		function notice(_str){ //提醒信息显示函数
			...
			var _tmp_timer=new Timer(4000,3)//设置定时器，3次移动提示一条文本后删除该提示文本
			_tmp_timer.addEventListener(TimerEvent.TIMER,function (){
				_errorTex.y-=18;  //每上移一步变透明一点
		...
		private function drawImgShower(_tmp_num) {//根据缩略图张数重绘上传显示框大小
			var _tmp_height
			var line_number = Math.ceil(_tmp_num/5);
			if(line_number<3){//小于2行时 整个UI界面高390，上传显示框高为256
				_tmp_height=_imgShowerH
				_UIHeight=330
				//_foot.y=_UIHeight-_footH
			}else{//大于2行时，每多一行上传框高+128，同时下部操作界面:_foot移动
				var tmp_add_height=(line_number-2)*133  
				_tmp_height = _imgShowerH + tmp_add_height
				_UIHeight=330+ tmp_add_height 
				//_foot.y=_UIHeight-_footH
			}
			
			ExternalInterface.call("setFlashHeight",_UIHeight);
			...
			*/
			_tmp_gp.lineStyle();
			//_tmp_gp.beginFill(0xFCCFA6);       //绘制小标题栏
			_tmp_gp.drawRect(0.5, 0.5, _UIWidth-5, 60);
			_tmp_gp.endFill();
			_tmp_gp.lineStyle();
			var matix:Matrix =new Matrix()//矩阵
			matix.createGradientBox(0, 35, 0,0,0);
			//_tmp_gp.beginFill(0xFCCFA6);       //绘制小标题栏
			var color =  root.loaderInfo.parameters.barColor;
			_tmp_gp.beginGradientFill("linear",[0xffffff,color ? color : 0xFCCFA6],[1,1],[0,255],matix);
			//_tmp_gp.lineto(0,10);
			_tmp_gp.drawRect(0, 0, _UIWidth, 35);
		}
