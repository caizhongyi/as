package {
	...
	*/
	public class YItem extends Sprite {
		...
		public  var _urlRequest;              //上传的URLRquest实例
		private var _cSize:uint=102;            //整个缩略图显示框大小
		...
		private var sizeLimit=2*1024*1024 //2M上传限制
		...
		public function YItem(_fr,_upURL="up.php",_sizeLimit=2*1024*1024) {
			...
			//标题输出文本域
			_title=new TextField();//状态输出文本域
			_title.x=10;
			_title.y=_cSize;
			_title.width=80;
			_title.height=20;
			_title.text=fileR.name;
			_title.selectable=false
			_tmp_tf.color=0xffffff; //字体格式
			_title.defaultTextFormat=_tmp_tf
			addChild(_title);
			...
			loadingBar.graphics.beginFill(0x97C6E7);
			loadingBar.graphics.drawRect(5, _cSize-60, _cSize-9, 13);
    ...
    private  function onOutCon(e:Event){//鼠标移到缩略图上将显示操作菜单
		...
		private function handleFileR() {//FileReference为单个文件引用类。用来处理单个文件这里为了简单而使用。
			...
			fileR.addEventListener(DataEvent.UPLOAD_COMPLETE_DATA, uploadCompleteDataHandler);//上传完毕事件
		...
		fileR.upload(_urlRequest,"image");//执行上传
		}//function
		...
		private function uploadCompleteDataHandler(event:DataEvent):void { 
			...
			uploadOver=true
			ExternalInterface.call("singleImageUploadComplete",event.data);
		...
		private function onFileExecute(e:Event) {//文件相关事件处理
			ExternalInterface.call("onFileExecute",e);
			switch (e.type) {
